...
დატასეტი შეიცავს 5 ფასდაკლების სვეტს, რომელიც სხვადასხვა ტიპის ფასდაკლებას არნიშნავენ. ამ სვეტებში ძალიან ხშირია მისინგ ველიუები, რაც აუცილებდლად არ ნიშნავს, რომ მაშინ ფასდაკლება არ ყოფილა, შეიძლება ინფორმაცია არ შეგროვებულა ან ეს ფასდაკლების ტიპი ზოგ მაღაზიაში ან დეპარტამენტში არ ყოფილა და ა.შ.
ასეთ შემთხვევაში ხუთივესთვის შევქმენი ახალი სვეტები - `MarkDown1_was_missing` (1/0), ეს ინფორმაციის შესანარჩუნებლად რომ დატა არ გვაქვს. შემდეგ მისინგ ველიუს მნიშვნელობას 0-ით ვცვლი.
...
- ეკონომიკური ინდიკატორები გარკვეულ გავლენას ახდენენ მომხმარებელთა ხარჯვის ქცევაზე
- მარტივი თარიღების მახასიათებლები ვერ ასახავს საცალო გაყიდვების რთულ ციკლურ ბუნებას
...
უფრო ბუნებრივია, რომ კონკრეტული ერთი დღესასწაულის დღის მაგივრად ზოგადად მაგ დღესასწაულამდე პერიოდი განვიხილოთ, როცა საქმე გაყიდვებს ეხება. ამას ვხედავთ გრაფიკებშიც, სადაც მთავარ დღესასწაულამდე გაყიდვაში უკვე ზრდას ვხედავთ. ამიტომ დავამატე დღესასწაულებამდე 1 კვირის (ეს ცვალებადია) აღმნიშნავი.
...
ბოლოს მაგეებზე ვქენი კიდე ერთხელ. ასევე ვნახეთ წინა პერიოდების ვაყიდვების კორელაცია ახლანდელ გაყიდვებთან და დავამატეთ 1, 4 და 52 კვირის ლად ფიჩერები.
...
#### განსხვავება სხვა მოდელებისგან:
- **No hyperparameter tuning**: default-ებით კარგად მუშაობს
- **Seasonality modes**: 'additive' vs 'multiplicative' (ავირჩიეთ multiplicative გაყიდვებისთვის), თუ ტრენდი გაორმაგდება, სეზონური ზრდის პიკებიც დაახლოებით გაორმაგდება, additive დროს ეფექტი იქნებოდა ფიქსირებული. 

თავიდან, როცა რეგრესორებს არ ვიყენებდი პრეპროცესინგის ნაწილი საერთოდ არ მქონდა, რადგან პროფეტი მისინგ მნიშვნელობებს თვითონ ხედავს და თან სეზონურობასაც თვითონ იჭერს, ამიტომ არ დამჭირვებია დამატებითი დროზე დამოკიდებული ფიჩერების შექმნა, უბრალოდ გადავეცი დღესასწაულების კალენდარი. Prophet ტრენინგის დროს რეგერესორების ნალ ველიუებს ვერ უმკლავდება, ასეთ სვეტებს არ იყენებს ტრენინგში, ამიტომ რეგრესორების დამატებისას (ტემპერატურა, ფასდაკლებები, უმუშევროვა, საწვავის ფასი და ა.შ.) პრეპროცესინგი დავამატე.
...
Prophet ერთ მოდელს არ იყენებს მთელ დატაზე, ის ყოფს მოდელებს ყველა მაღაზია-დეპარტამენტი კომბინაციისთვის:


#### Fallback Strategy
ტრენინგს ვაკეთებდი ისეთ მაღაზია-დეპარატამენტის კომბინაციებზე სადაც დატა 10ზე მეტი იყო. ამიტომ შემდეგ ეს დასქიფული დეპარტამენტები ცალკე შევინახე, რომლებისთვისაც მოვძებნე იგივე მაღაზიაში ყველაზე ახლოს მყოფი დეპარტამენტი და მისი შედეგი ავიღე ამის მაგივრად. გავტესტე ეს მიდგომაც, რათა მაქსიმალურად გამომეყენებინა დატა,იმ შემთხვევაში თუ მცირე მონაცემები მნიშვნელოვანი იქნებოდა საბოლოო შედეგისთვის, თუმცა ტრენინგის დროს ვნახე, რომ თვითონს ეს სტრატეგიაც ისეთ ადეკვატურ შედეგს არ მაძლევდა, როგორსაც ველოდი (დიდი შეცდომა ჯდებოდა) და შემდეგ მის გარეშე რომ დავატრენინგე დიდი გავლენა მანდაც ვერ დავინახე. 
...
| თვისება | Deep Learning (N-BEATS, DLinear) | კლასიკური ML (XGBoost, LightGBM) | Time Series მოდელები (ARIMA, SARIMA, Prophet) |
|---|---|---|---|
